...
db_uri = f"mysql://{db_username}:{db_password}@{db_host}:{db_port}"
...
app = Flask(__name__)
app.config['SERVER_NAME'] = 'backend.locationpro.app'
# Configure SSLify to enforce HTTPS (optional but recommended)
...
def create_user():
    ...
    try:
        ...
        access_token = create_access_token(identity={'id': user_add.id, 'type': user_add.type}, expires_delta=timedelta(minutes=15))

        expiration_time = datetime.utcnow() + timedelta(minutes=15)
        expiration_time_unix = int(expiration_time.timestamp()) * 1000

        # Return the access token and user data in JSON format
        return jsonify({'access_token': access_token, 'expiration_time': expiration_time_unix, 'user': user_add.serialize()}), 200
...
def login():
    ...
    try:
        ...
        print(expiration_time_unix)
        # Return the access token and user data in JSON format
        return jsonify({'access_token': access_token, 'expiration_time': expiration_time_unix, 'user': existing_user.serialize()}), 200
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000) #running on locationpro.app - change to Debug/remove port for production purposes